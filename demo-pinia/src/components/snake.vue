<template>
  <div>
    <canvas id="ctx" :width="width" :height="height"></canvas>
  </div>
</template>
...
import { onMounted , ref } from "vue";
    ...
    let direction = 'right' // top/down/left/right  当前方位
    let elementWidth = 10 //元素尺寸
    let step = 10 //每一步进行距离
    let store  = ref(0); // 分数
    let status = ref('start'); //状态 unStart/start/pause/over/success
    let foodCoordinate:any = [
        ((Math.random()*width.value/10))|0,
        ((Math.random()*width.value/10))|0 
    ] //食物的位置，随机生成
    let process:any = null //定时器

    //初始化操作,在页面渲染完毕即onMount生命周期
    function handleInit(){
